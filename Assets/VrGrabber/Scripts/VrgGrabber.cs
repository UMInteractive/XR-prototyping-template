...
public class VrgGrabber : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        UpdateGrab();
            ...
            if (Physics.Raycast(ray, out hitObject, maxGrabDistance, layerMask))
            {
                
                    hitObj = hitObject.collider.gameObject;
                Transform tempSize = hitObj.transform;
                if(!sizeIncreased)
                {
                    hitObj.transform.localScale = hitObj.transform.localScale * 1.4f;
                    sizeIncreased = true;
                }
